...
class TransitionTest {
    ...
    void testIsEpsilon() {
        ...
        assertFalse(normalTransition.isEpsilon(), "A transition with a non-null symbol must not be considered epsilon");

        // Epsilon transition (null symbol)
        Transition epsilonTransition = new Transition(q0, null, q1);
        assertTrue(epsilonTransition.isEpsilon(), "A transition with a null symbol must be considered epsilon");
    }
    ...
    void testRecordAccessors() {
        ...
        assertEquals(startState, transition.from(), "The from() accessor must return the correct start state");
        assertEquals('x', transition.symbol(), "The symbol() accessor must return the correct symbol");
        assertEquals(endState, transition.to(), "The to() accessor must return the correct destination state");
    }
}