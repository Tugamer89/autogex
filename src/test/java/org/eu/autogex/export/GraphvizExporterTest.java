...
class GraphvizExporterTest {
    ...
    void testExportDFA() {
        ...
        assertTrue(
                dot.contains("node [shape = doublecircle]; \"q1\";"),
                "q1 must be a double circle (final state)");
        assertTrue(dot.contains("__start0 -> \"q0\";"), "Must have an entry arrow to q0");
        assertTrue(
                dot.contains("\"q0\" -> \"q1\" [label=\"a\"];"),
                "Must contain the transition from q0 to q1");
    ...
    void testExportENFAWithEpsilon() {
        ...
        assertTrue(
                dot.contains("\"A\" -> \"B\" [label=\"ε\"];"),
                "Epsilon transitions must be labeled with ε");
    }
